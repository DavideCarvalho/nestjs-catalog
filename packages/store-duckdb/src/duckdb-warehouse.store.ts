...
export class DuckDbWarehouseStore {
  ...
  async countStaged(type: CatalogObjectTypeDef, snapshotId: string): Promise<number> {
    ...
    // `union_by_name = true`, not the default. A later task writes carry-forward rows as
    // their own object in the same snapshot prefix with a different column order (and,
    // eventually, a different column set), and `read_parquet` over a glob does not union
    // mismatched schemas by name unless told to — verified against the real engine: without
    // this, a glob whose files disagree on which columns exist silently drops the columns
    // some of them lack rather than erroring, so the fault would surface as missing data far
    // from its cause. With it, columns are matched by name across every file in the glob.
