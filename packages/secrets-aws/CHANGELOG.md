...
- 817e100: Stop asserting that random ciphertext does not contain two letters
...
- a92ede1: A KMS vault, so the credential in `config` rests encrypted
  ...
  value that is _not_ one of these is refused by name rather than sliced into
  ...
  **every secret sealed unopenable, from the first one**, surfacing as an opaque
  ...
  Recording in the envelope _whether_ the id was bound does work, and was rejected:
  ...
  half in isolation, against a fake KMS that does _not_ enforce the context, because
  ...
  _and_ the encryption context so a hit can never stand in for the check. The secret
