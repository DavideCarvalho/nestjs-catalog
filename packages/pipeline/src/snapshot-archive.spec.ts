...
describe('archiveColumns', () => {
  ...
  /**
   * `_batch` is the one system column with a meaning that is not recoverable
   * from anywhere else: `-1` marks rows an incremental run carried forward
   * rather than produced, and the SQL console offers that as a question people
   * ask of a load. It is not archived.
   *
   * That is a real loss and it costs nothing while nothing is deleted, because
   * the rows are still in the table. It becomes load-bearing the moment an
   * eviction removes them, so this test exists to make the gap visible now
   * rather than to bless it.
   */
  it('does not preserve _batch, which anything that deletes rows would have to', () => {
    const columns = archiveColumns(typeOf([{ name: 'hours', type: 'number' }]));
    expect(columns.map((column) => column.name)).not.toContain('_batch');
  });
});
