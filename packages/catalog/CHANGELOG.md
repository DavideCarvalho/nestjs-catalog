...
- b331837: Point `physicalColumn`'s note at `warehouse.store.ts`, the file that actually holds the copy it describes — `mysql-warehouse.store.ts` was renamed when the Postgres warehouse store landed.

## 0.34.0

### Minor Changes

- 52cb715: A retention sweep asks which snapshots still hold rows, instead of filtering a bounded list of records
