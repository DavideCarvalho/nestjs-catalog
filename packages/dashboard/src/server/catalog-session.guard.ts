...
import { DASHBOARD_BASE_PATH } from './catalog-ui.controller.js';

/**
 * Gates `CatalogUiController` (the SPA shell + assets, a full-page browser navigation) on a valid
 * `dashboardAuth` session cookie. A no-op (always `true`) when `dashboardAuth` was not configured
 * — see `CatalogDashboardModule.forRoot`'s `dashboardAuth` doc. Missing/invalid/expired session,
 ...
 * (see the catalog dashboard's topology note in `dashboard.service.ts`) — it only ever answers
 * "is there a valid dashboard session", nothing role-specific changes here.
 */
@Injectable()
export class CatalogUiSessionGuard implements CanActivate {
  constructor(
