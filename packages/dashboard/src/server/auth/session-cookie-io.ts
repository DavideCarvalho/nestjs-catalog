...
export const SESSION_COOKIE_NAME = 'catalog_dashboard_session';

/**
 * `Path=/`, deliberately not scoped to `basePath`/`apiBasePath`: the two mounts are independently
 * configurable (`CatalogDashboardModule.forRoot({ basePath, apiBasePath })`) and can live at
 * unrelated paths (e.g. a `/catalog` UI + an `/api/catalog-service` API) — a cookie scoped to either one alone
