 ...
 * under `basePath` alongside `CatalogUiController` (see `CatalogDashboardModule.forRoot`), but on
 * a SEPARATE controller so it is never itself behind `CatalogUiSessionGuard` — these endpoints
...
export class CatalogAuthController {
  private readonly logger = new Logger(CatalogAuthController.name);
