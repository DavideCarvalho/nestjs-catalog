...
const DEFAULT_BASE_PATH = '/catalog';
 ...
 */
export function catalogConsoleSessionUrl(basePath: string = DEFAULT_BASE_PATH): string {
  return `${normalizeBasePath(basePath)}/session`;
}

/** Where the console SPA itself is served. */
export function catalogConsoleUrl(basePath: string = DEFAULT_BASE_PATH): string {
 ...
 */
export async function mintCatalogConsoleSession(options: OpenConsoleOptions = {}): Promise<void> {
  const url = catalogConsoleSessionUrl(options.basePath);
 ...
 */
export async function openCatalogConsole(options: OpenConsoleOptions = {}): Promise<void> {
  await mintCatalogConsoleSession(options);
  const target = catalogConsoleUrl(options.basePath);
