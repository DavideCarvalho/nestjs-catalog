...
/**
 * React layer over `openCatalogConsole` — the middle tier between the bare function
 * (`../client/console-session.ts`) and the drop-in `<OpenCatalogConsoleButton>`.
 *
 * You get the state a launcher UI actually needs (in-flight, error) and keep full control of the
 * markup. Nothing here is Catalog-specific beyond the endpoint: it is a mint-then-navigate call with
...
export function useOpenCatalogConsole(options: OpenConsoleOptions = {}): UseOpenConsoleResult {
  ...
  const open = useCallback(() => {
    setIsPending(true);
    setError(null);
    void openCatalogConsole(optionsRef.current)
      .then(() => {
 ...
 *
 * ```ts
 * const { mutate, isPending } = useMutation(openCatalogConsoleMutationOptions({ headers }));
 * ```
 */
export function openCatalogConsoleMutationOptions(options: OpenConsoleOptions = {}): {
  mutationKey: readonly unknown[];
  mutationFn: () => Promise<void>;
} {
  return {
    mutationKey: ['catalog', 'console', 'open', options.basePath ?? null] as const,
    mutationFn: () => openCatalogConsole(options),
  };
}
